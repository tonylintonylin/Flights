...
// Template on local pages
FlightsNamespace.namespace("Local").Doit = (function () {
  var start = function () {};

  return { start: start };
})();

// Display failure and success alert boxes
FlightsNamespace.namespace("Utils").Alert = (function () {
  var start = function () {
    $("#alert-success")
      .fadeIn(1000)
      .delay(3000)
...
FlightsNamespace.namespace("Utils").Delete = (function () {
  var start = function () {
    ...
    $(".js-delete").on("click", function (e) {
      if ($(this).attr("href") != "javascript:void(0);") {
        ...
        // Open and populate modal delete box
        $("#delete-id").val(id);
...
FlightsNamespace.namespace("Utils").Misc = (function () {
  var start = function () {
    // Dctivate proper tab when returning to detail pages
    ...
    // Filter toggles animate to different filter area
...
FlightsNamespace.namespace("Utils").Import = (function () {
  var start = function () {
...
FlightsNamespace.namespace("Utils").Typeahead = (function () {
          ...
          },
          {
            ...
            templates: {
              suggestion: Handlebars.compile(
                '<div><i class="{{icon}} icon-little"></i> {{name}}</div>'
              ),
            },
          }
        )
...
$(function () {
  FlightsNamespace.Utils.Alert.start();
  FlightsNamespace.Utils.Misc.start();
  FlightsNamespace.Utils.Delete.start();
  FlightsNamespace.Utils.Typeahead.start();
});
